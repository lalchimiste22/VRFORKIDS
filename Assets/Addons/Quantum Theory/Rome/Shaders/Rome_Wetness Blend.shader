Shader "Quantum Theory/PBR - NonBump Wetness Blend (VC Red,Green,Alpha)" {
	Properties{
...
#pragma target 3.0
	...
	void surf(Input IN, inout SurfaceOutputStandard o) {
		...
		fixed4 c2 = tex2D(_MainTexB, IN.uv_MainTexB);
		fixed4 m = tex2D(_MetallicGlossMap, IN.uv_MetallicGlossMap);
		fixed4 m2 = tex2D(_MetallicGlossMapB, IN.uv_MainTexB);
