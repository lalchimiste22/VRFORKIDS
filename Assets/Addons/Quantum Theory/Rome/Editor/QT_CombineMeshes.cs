...
public class QT_CombineMeshes : EditorWindow
{
    ...
    //private static string pathRelative = "Assets/CombinedMesh_Prefabs/";
   ...
   // private int layer = 0;
    ...
    private int objectCount = 0; //counter for the autonamed objects. Appended to the end of the name.
    ...
    private void OnGUI()
    {
        ...
        if (matcount > 0)
        {
            ...
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("Export Path: ", pathRelative);
            EditorGUILayout.EndHorizontal();
            if (GUILayout.Button("Set Path"))
            {
                pathRelative = EditorUtility.SaveFolderPanel("Choose Folder in which to Export Combined Meshes", "", "");
                pathFull = pathRelative;
                pathRelative = pathRelative.Replace(Application.dataPath, "Assets");               
            }
            ...
            {
                bool check = true;
                //set folder name if autoname is true
                
                if (pathRelative == "" || pathRelative == null)
                ...
                else
                {
                    ...
                    if (AutoName)
                    {
                        //since autoname is true,g et all the prefabs in the folder. split the array up by hyphen if any prefabs exist
                        string[] files = Directory.GetFiles(pathFull);
                        List<string> Prefabs = new List<string>();
                        foreach(string s in files)
                        {
                            if (s.Contains(".prefab") && !s.Contains(".meta"))
                            {
                                
                                string[] t = s.Split('/');
                                string temp = t[t.Length - 1];
                                t = temp.Split('-'); //split by hypen
                                temp = t[0];
                                t = temp.Split('\\');
                                Prefabs.Add(t[t.Length-1]);
                            }
                        }

                        if(Prefabs.Count>0)
                        {
                            int[] intIndex = new int[Prefabs.Count];
                            //now we have MeshX where X is a number
                            for(int x=0;x<Prefabs.Count;x++)
                            {
                                string z = Prefabs[x].Replace("Mesh", "");                               
                                intIndex[x] = Convert.ToInt16(z);
                            }
                            objectCount = 1 + intIndex.Max();
                    ...
                    {
                        if (Selection.gameObjects[x].transform.localScale.x < 0 || Selection.gameObjects[x].transform.localScale.y < 0 || Selection.gameObjects[x].transform.localScale.z < 0)
                        {
                            EditorUtility.DisplayDialog("Negatively Scaled Object", Selection.gameObjects[x].name + " is negatively scaled which will not convert properly. Exclude this object from the selection and try again.", "OK");
                            check = false;
                        }
                        else
                            check = true;
                    ...
                    if (check)
                    {
                        newObjects.Clear();
                      //  if (AutoName)
                       //     objectCount = objectCount + startIndex;
                        Combine();
    ...
    private void WritePrefab()
    {
        ...
        foreach (GameObject g in newObjects)
        {
            ...
            m.name = g.name;
            AssetDatabase.CreateAsset(m, pathRelative + "/" + m.name + ".asset");
        }
        PrefabUtility.CreatePrefab(pathRelative + "/" + newParent.name + ".prefab", newParent);
        
        GameObject prefab_go = PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath(pathRelative + "/" + newParent.name + ".prefab", typeof(GameObject))) as GameObject;
    ...
    private void CreatePrefabFromSelected()
    {
       ...
       // bool doesExists = System.IO.Directory.Exists(pathRelative);
       // if (!doesExists)
       //     System.IO.Directory.CreateDirectory(pathRelative);
        //save assets
           ...
           // Debug.Log("Trying to Convert " + name + " into Prefab");
            ...
            if (lemesh != null)
            {
                if (lemesh.sharedMesh.name != "Combined Mesh")//ensuring combined mesh
                {
                    ...
                    AssetDatabase.CreateAsset(lemesh.sharedMesh, pathRelative + "/"+name + ".asset");
            }
            else
            {
                continue;
            ...
            PrefabUtility.CreatePrefab(pathRelative+"/"+ name + ".prefab", selectionmeshes[i].gameObject);
            ...
            GameObject prefab_go = PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath(pathRelative+"/" + name + ".prefab", typeof(GameObject))) as GameObject;
            //ensure prefab replacement in scene
            prefab_go.transform.parent = parent_tr;
            prefab_go.transform.position = child_pos;
        }
        AssetDatabase.Refresh();
        Debug.Log("Combined Meshes located in " + pathRelative);
    }
}
